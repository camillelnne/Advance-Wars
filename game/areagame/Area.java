...
public abstract class Area implements Playable, PauseMenu.Pausable {
    ...
    protected List<Unit> getRegisteredUnits(){
     ...
     * if the actor is a Unit, will also be added to a list of registered Units
     * @param a (Actor): the actor to register, not null
     * @return (boolean): true if the actor is correctly registered
     */
    public final boolean registerActor(Actor a){
        if(a instanceof Unit){
            registeredUnits.add((Unit)a);
        }
    ...
    public boolean begin(Window window, FileSystem fileSystem) {
        ...
        for (Unit u:registeredUnits){
            System.out.println(u.getSpriteName());
        }
